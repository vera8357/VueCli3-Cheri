...
export default {
  ...
  methods: {
    getCoupons (page = 1) {
      const url = `${process.env.VUE_APP_PATH}/api/${process.env.VUE_APP_CUSTOMPATH}/admin/coupons?page=${page}`
      let vm = this
    ...
    openCouponModel (data, status) {
      let vm = this
      if (status) {
        vm.tempCoupon = JSON.parse(JSON.stringify(data))
      } else {
        vm.tempCoupon = {}
      }
      vm.modifyCoupon = status
      $('#couponModal').modal('show')
    },
    updataCoupon () {
      let vm = this
      let url = `${process.env.VUE_APP_PATH}/api/${process.env.VUE_APP_CUSTOMPATH}/admin/coupon/${vm.tempCoupon.id}`
      let httpMethod = 'put'
      let message = '已更新優惠卷'
      let status = 'success'
      if (!vm.modifyCoupon) {
        url = ` ${process.env.VUE_APP_PATH}/api/${process.env.VUE_APP_CUSTOMPATH}/admin/coupon`
        httpMethod = 'post'
        message = '已新增優惠卷'
      }
    ...
    openDelCouponModal (item) {
      let vm = this
      vm.delCoupon = Object.assign({}, item)
      $('#delModal').modal('show')
    },
    deleteCoupon (id) {
      const url = `${process.env.VUE_APP_PATH}/api/${process.env.VUE_APP_CUSTOMPATH}/admin/coupon/${id}`
      let vm = this
