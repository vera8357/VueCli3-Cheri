...
export default {
  ...
  methods: {
    getOrders (page = 1) {
      const url = `${process.env.VUE_APP_PATH}/api/${process.env.VUE_APP_CUSTOMPATH}/admin/orders?page=${page}`
      let vm = this
    ...
    openOrderModel (data, status) {
      let vm = this
      vm.tempOrder = JSON.parse(JSON.stringify(data))
      vm.modifyOrder = status
      $('#orderModal').modal('show')
    },
    updatedOrder () {
      let vm = this
      const url = `${process.env.VUE_APP_PATH}/api/${process.env.VUE_APP_CUSTOMPATH}/admin/order/${vm.tempOrder.id}`
      let message = '已更新訂單內容'
      let status = 'success'
