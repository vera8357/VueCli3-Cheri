...
export default {
  ...
  methods: {
    getPageProduct (page = 1) {
      let vm = this
      const url = `${process.env.VUE_APP_PATH}/api/${process.env.VUE_APP_CUSTOMPATH}/products?page=${page}`
    ...
    getALLProduct () {
      let vm = this
      const url = `${process.env.VUE_APP_PATH}/api/${process.env.VUE_APP_CUSTOMPATH}/products/all`
      vm.$http.get(url).then(res => {
        if (res.data.success) {
